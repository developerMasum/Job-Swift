...
const PeopleSearch = () => {
  ...
  const options = [{ level: "Keywords" }, { level: "ABC" }, { level: "XYZ" }];
  ...
  useEffect(() => {
    const URL = ` https://server-wheat-beta.vercel.app/all-applications2?search=${search}`;
    fetch(URL)
              ...
              <div>
                {application?.map((data, index) => {
                  return (
                    <div
                      key={index}
                      className="card card-side bg-green-100 shadow-md rounded-md my-8"
                    >
                      <figure>
